...
| FR-05 | The combination `verbose` plus `technical` must inject no paragraph, exactly as `verbose` does today. | confirmed | `hooks/session-start#four outcomes, and the silent one is deliberate`, and the measured 300-token verbose form |
...
| ID | Requirement | Status | Evidence |
|---|---|---|---|
| NFR-01 | No combination of `response_style` and `explain_level` may produce an injection larger than 1,285 characters, which is 356 tokens by the estimate `tests/validate-skills.sh` uses. | confirmed | Requester chose this ceiling over the 400 hard limit, 2026-08-18, when `terse` cost 1,284 characters. **The ceiling has not moved; the cost under it has, twice, and this cell recorded neither until 2026-09-02.** It fell to 1,266 on 2026-08-30, when adding `design-database` to the router needed 17 characters against the 1 spare and four phrases were trimmed to pay for it. It rose to **1,279** on 2026-09-02, when the hook began printing the loaded plugin version, which costs 13. Six characters remain. A dated figure left in place is trusted long after the date stops meaning anything, which is what happened here and in the four-form table `NFR-05` names |
| NFR-02 | `tests/validate-skills.sh` must apply the size check to every combination, not only to the one the repository's own profile happens to select. | confirmed | `tests/validate-skills.sh`, in the block opening `if [ -f hooks/session-start ]; then`, writes a profile for each of the **all four combinations** of `response_style` and `explain_level` into a `mktemp` probe directory and runs the hook in each, so all four are measured. **Corrected 2026-09-09, and the original wording is kept here rather than overwritten**, as NFR-01 and NFR-03 keep theirs. It read "runs the hook once from the repository root, so it measures `terse` plus `technical` and nothing else", which described this requirement failing while the row said `confirmed`. The loop landed with the four-form work and neither this cell nor the story's Notes moved with it |
| NFR-03 | The hook's output must change only following a deliberate act, so that it costs one cache miss on the session after that act rather than one per request. A profile edit and a plugin upgrade are both such acts; nothing else may move the bytes. | confirmed | `hooks/session-start:4-14`. **Widened 2026-09-02, and the original wording is kept here rather than overwritten.** It read "must stay byte-identical for a given profile, changing only when a person edits the profile", with the evidence "a fourth form is within the bound that comment sets; a value read for its content would not be". That was a form-count bound and it held for `response_style`, which has four enumerated forms. It could not admit the loaded plugin version, which is a value read for its content with one form per release. The requester chose to widen rather than to take an exception, 2026-09-02, because a change-frequency bound covers both cases under one rule and the version changes strictly less often than the profile does: the cache path is versioned, so an upgrade writes a new directory rather than editing an installed copy |
...
| NFR-05 | `docs/05-token-and-memory-design.md` must state the measured size of each of the four forms, replacing the two figures it carries today. | confirmed | The four-form table under "The rule is on by default and it is not free", which is the record of the tightest budget in the project. Cited by line number until 2026-09-02, when the pointer had drifted onto a passage about the handoff file and the table it named was still carrying the 2026-08-18 figures that the 2026-08-30 trim had superseded. A line number is not a durable citation in a file that grows; the heading is |
